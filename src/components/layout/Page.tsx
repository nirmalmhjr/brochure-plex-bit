 ...
 */
export default function Page({ children, className = "", showAccent = true }: PageProps) {
  return (
      ...
      {showAccent && (
        <>
          <div
            ...
            style={{ background: "radial-gradient(circle, rgba(126,34,206,.16), transparent 70%)" }}
          />
          <div
            aria-hidden
            className="pointer-events-none absolute -bottom-28 -left-28 size-80 rounded-full opacity-50"
            style={{ background: "radial-gradient(circle, rgba(126,34,206,.10), transparent 70%)" }}
          />
          <div aria-hidden className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-brand-deep via-brand to-fuchsia-400" />
        </>
      )}
      {children}
    </section>
